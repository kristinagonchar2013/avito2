...
public class FavoriteDataInit {
    ...
    private void favouriteDataInit() {
        favoriteDao.persist(Favorite.builder()
                .items(List.of(itemDao.findById(1L), itemDao.findById(2L)))
                .shops(List.of(shopDao.findById(1L)))
                .build());

        favoriteDao.persist(Favorite.builder()
                .items(List.of(itemDao.findById(3L), itemDao.findById(4L), itemDao.findById(5L)))
                .build());

        favoriteDao.persist(Favorite.builder()
                .shops(List.of(shopDao.findById(2L), shopDao.findById(3L), shopDao.findById(4L)))
                .build());

        favoriteDao.persist(Favorite.builder()
                .items(List.of(itemDao.findById(6L), itemDao.findById(7L), itemDao.findById(8L)))
                .shops(List.of(shopDao.findById(4L)))
                .build());
    }
}

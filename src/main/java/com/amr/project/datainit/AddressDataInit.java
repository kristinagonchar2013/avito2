...
public class AddressDataInit {
    ...
    private void addressDaoInit() {
        addressDao.persist(Address.builder()
                .id(1L).cityIndex("12345").city(cityDao.findById(2L))
                .street("Oktyabrskaya").house("21").build());

        addressDao.persist(Address.builder()
                .id(2L).cityIndex("12233").city(cityDao.findById(1L))
                .street("Arbat").house("1").build());

        addressDao.persist(Address.builder()
                .id(3L).cityIndex("09987").city(cityDao.findById(3L))
                .street("Ivana Franko").house("45").build());

        addressDao.persist(Address.builder()
                .id(4L).cityIndex("34567").city(cityDao.findById(4L))
                .street("Deribasovskaya").house("34").build());

        addressDao.persist(Address.builder()
                .id(5L).cityIndex("44444").city(cityDao.findById(5L))
                .street("Dolce").house("76").build());

        addressDao.persist(Address.builder()
                .id(6L).cityIndex("55555").city(cityDao.findById(6L))
                .street("Vita").house("1").build());

        addressDao.persist(Address.builder()
                .id(7L).cityIndex("66666").city(cityDao.findById(7L))
                .street("Revolution street").house("82").build());

        addressDao.persist(Address.builder()
                .id(8L).cityIndex("77777").city(cityDao.findById(8L))
                .street("Baker street").house("101").build());

        addressDao.persist(Address.builder()
                .id(9L).cityIndex("88888").city(cityDao.findById(9L))
                .street("Main Street").house("1221").build());

        addressDao.persist(Address.builder()
                .id(10L).cityIndex("99999").city(cityDao.findById(10L))
